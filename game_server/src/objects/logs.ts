...
export const  roundResultsToFireBaseObject = (roundResults: RoundResults): any => {
  return {
...
export const roundLogsToFireBaseObject = (roundLogs: RoundLogs): any => {
  return {
    predictionInfo: roundLogs.predictionInfo.map(predictionLogToFireBaseObject),
    playActionInfo: roundLogs.playActionInfo.map(playActionLogToFireBaseObject),
    handWinnerInfo: roundLogs.handWinnerInfo.map(handWinnerLogToFireBaseObject),
    roundResultInfo: roundLogs.roundResultInfo.map(roundResultLogToFireBaseObject),
    linearLogs: roundLogs.linearLogs,
  }
}

export const predictionLogToFireBaseObject = (predictionLog: PredictionLog): any => {
  return {
...
export const playActionLogToFireBaseObject = (playActionLog: PlayActionLog): any => {
  return {
...
export const handWinnerLogToFireBaseObject = (handWinnerLog: HandWinnerLog): any => {
  return {
    handWinnerPlayerId: handWinnerLog.handWinnerPlayerId,
    handWinnerCard: handWinnerLog.handWinnerCard.toFirebaseObject(),
    handsCardsInOrder: handWinnerLog.handsCardsInOrder.map( (card) => card.toFirebaseObject()),
  }
}

export const roundResultLogToFireBaseObject = (roundResultLog: RoundResultLog): any => {
  return {
    ...
    originalHand: roundResultLog.originalHand.map( (card) => card.toFirebaseObject()),
  }
}