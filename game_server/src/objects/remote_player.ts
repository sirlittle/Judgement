...
import { Card, cardInHand, Predictions, HandCounter } from 'judgement_utils';
...
export class RemotePlayer extends RegulatedPlayer {
    ...
    async playCard(
        cardsPlayedInOrder: Card[],
        handsWonByPlayer: HandCounter,
        roundPredictions: Predictions,
        trumpCard: Card
    ) {
        // Use url to get card from remote player, make sure card is within the legal moves and then return it
        const remotePlayerResp = await fetch(this.url + '/playCard', {
            // need to ensure that the type returned is a card
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const cardToPlay = new Card(
            remotePlayerResp.card.suit,
            remotePlayerResp.card.rank
        );
    ...
    async predict(predictions: Predictions, trumpCard: Card): Promise<number> {
        ...
        const sumOfPredictions = Object.values(predictions).reduce(
            (a, b) => a + b,
            0
        );
        const lastPlayer =
            numPlayersAlreadyPredicted === this.getNumberOfPlayers() - 1;
        if (
            sumOfPredictions + prediction == this.getNumOfCards() &&
            lastPlayer
        ) {
            if (prediction > 0) {
                return prediction - 1;
