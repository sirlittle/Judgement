...
app.post('/game', async (req, res) => {
  ...
  const numberOfPlayers = parseInt(req.query.numberOfPlayers?.toString() || "5");
  const gameResults: string[] = await simulateGames(numberOfGames, numberOfPlayers);
...
export const index: HttpFunction = async (req, res) => {
  ...
  switch(req.path){
    ...
    case "/getDemoHand":
      ...
      break;
    case "/getGameScore":
      const gameInfo = await getGameScore(req.query.gameId?.toString() || "");
...
export const simulateGames = async (numberOfGames: number, numberOfPlayers: number): Promise<string[]> => {  
